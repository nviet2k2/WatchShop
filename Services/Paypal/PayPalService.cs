...
namespace Services.Paypal;
    ...
    public class PayPalService : IPayPalService
    {
        ...
        public async Task<string> CreatePaymentUrl(CreateOrderDTO model, HttpContext context)
        {
            ...
            var payment = new PayPal.v1.Payments.Payment()
            {
                ...
                Transactions = new List<Transaction>()
                {
                    new Transaction()
                    {
                        Amount = new Amount()
                        {
                            Total = ConvertVndToDollar(model.TotalPrice).ToString(),
                            Currency = "USD",
                            Details = new AmountDetails
                            {
                                Tax = "0",
                                Shipping = "0",
                                Subtotal = ConvertVndToDollar(model.TotalPrice).ToString(),
                            }
                        },
                        ItemList = new ItemList()
                        {
                            Items = new List<Item>()
                            {
                                new Item()
                                {
                                    Name = " | Order: " + model.Note,
                                    Currency = "USD",
                                    Price = ConvertVndToDollar(model.TotalPrice).ToString(),
                                    Quantity = 1.ToString(),
                                    Sku = "sku",
                                    Tax = "0",
                        ...
                        Description = $"Invoice #{model.Note}",
